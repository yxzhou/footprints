...
public class LargestBST
{
  ...
  public Hashtable<TreeNode, Integer> htC = new Hashtable<>();  // record(Node, BST sub tree node amount )
  public Hashtable<TreeNode, Integer> htMax = new Hashtable<>(); // record(Node, the max of this sub tree)
  public Hashtable<TreeNode, Integer> htMin = new Hashtable<>(); // record(Node, the min of this sub tree)
...
//  }
  ...
  private void inorderBST_R(TreeNode p){
    if (p != null) {
      if( p.left !=null && Integer.valueOf(p.val) > getMax(p.left) )
        inorderBST_R(p.left);
      
      BinaryTree.visit(p);
      
      if(p.right!=null && Integer.valueOf(p.val) < getMin(p.right) )
