...
public class DistinctSubsequences
{
  ...
  public int numDistinct_dp2(String S, String T) {
    ...
    for (int j = tLen - 1; j > -1; j--) {
      for (int i = sLen - 1 ; i > -1; i--){
        mt[i][j] += mt[i + 1][j] + (S.charAt(i) == T.charAt(j) ? mt[i + 1][j + 1] : 0);
      }
    }
  ...
  /* Wrong !! */
  public int numDistinct_dp2_n(String S, String T) {
  ...
  public static void main(String[] args) {
    ...
    for(int i=0; i<S.length; i++){
      System.out.println("\nInput :"+ S[i] + " " + T[i]);
      System.out.println("\tOutput:" + sv.numDistinct_recursive(S[i], T[i]) + "  " + sv.numDistinct_dp2(S[i], T[i]) + "  " + sv.numDistinct_dp_n(S[i], T[i]));
