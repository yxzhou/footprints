...
window.pg || $(document).ready( function($) {
    ...
    if ( wgCanonicalNamespace === '' || wgCanonicalNamespace === 'Project' || wgCanonicalNamespace === 'Help' ) {
        ...
        if( settings[0] == 0 ) {
            ...
            footer.append($("<li>").append($("<a>").text("启用参考文献提示工具").attr("href","javascript:(function(){})()").click(function(){toggleRT(1)})));
            return;
        }
        ...
        $(".reference").each( function() {
            ...
            function hide( refLink ){
                    if( tooltipNode && tooltipNode.parentNode == document.getElementById('bodyContent') ) {
                            hideTimer = setTimeout( function() {
                                    $(tooltipNode).animate({opacity: 0}, 100, function(){ document.getElementById('bodyContent').removeChild( tooltipNode ) })
            ...
            function show(){
                    if( !tooltipNode.parentNode || tooltipNode.parentNode.nodeType === 11 ){
                            document.getElementById('bodyContent').appendChild( tooltipNode );
                    }
            ...
            function openSettingsMenu(){
                    if( settingsMenu ) {
                            ...
                            settingsMenu = $("<form>").append(
                                    $("<button>").css("width","100%").text("关闭参考文献提示工具").button().click(function(){toggleRT(0)}),
                                    "<br>",
                                    $("<small>").text("一旦关闭，参考文献提示工具还可以从页面底部的一个链接重新启用。"),
                                    "<hr>",
                                    $("<label>").text("出现提示的延迟时间(毫秒): ").append($("<input>").attr({"type":"number","value":settings[1],step:50,min:0,max:5000})),
                                    "<br>",
                                    $("<span>").text("弹出提示的方式："),
                                    $("<label>").append(
                            ...
                            ).submit(function(e){e.preventDefault()}).dialog({modal:true,width:500,title:"参考文献提示工具选项",buttons:{"保存设置":function(){
            ...
            $(this)[ isTouchscreen ? 'click' : 'hover' ](function( e ){
                    ...
                    if( isTouchscreen ) {
                            e.preventDefault();
                            (tooltipNode && tooltipNode.parentNode == document.getElementById('bodyContent')) || setTimeout( function(){
                                    $( document.getElementById('bodyContent') ).bind("click touchstart", function( e ) {
                                            e = e || event;
                                            ...
                                            if( !e ){
                                                    ...
                                                    hide( _this );
                                                    $(document.getElementById('bodyContent')).unbind("click touchstart", arguments.callee)
                                            }
                                    })
                            }, 0);
                    }
                    ...
                    showTimer = setTimeout( function() {
                            var h = findRef( _this );
                            if( !h ){return};
                            if( !isTouchscreen && ( window.pageYOffset || document.documentElement.scrollTop || document.getElementById('bodyContent').scrollTop || 0 ) + $(window).height() > $( h ).offset().top + h.offsetHeight ) {
                            ...
                            show();
                            var o = $(_this).position(), oH = tooltipNode.offsetHeight;
