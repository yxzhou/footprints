 ...
 */
package fgafa.datastructure.intervaltree.justAHook;
...
public class Main1
{
  ...
  final static boolean[] isPartColored = new boolean[maxN << 2]; //default it's false. 
  
  final static int[] inputStart = new int[maxN];
  final static int[] inputEnd = new int[maxN];
  final static int[] inputType = new int[maxN];
  ...
  private int cal(int l, int r, int rt) {
    if(color[rt] > DEFALT )
      return (color[rt] - CUPREOUS) * (r-l+1); 
    if (!isPartColored[rt]) 
      return 0;
    ...
    int mid = (l + r) >> 1;
    int leftSon = rt << 1;
    ret += cal(l, mid, leftSon);// build(lson);
    ret += cal(mid + 1, r, leftSon + 1);// build(rson); rt << 1 | 1
  ...
  private void update(int L, int R, int typeId, int l, int r, int rt) {
    ...
    if (!isPartColored[rt] && L <= l && r <= R) {
      color[rt] = typeId;
      return;
    }
    
    isPartColored[rt] = true;
      ...
      while(in.hasNext()){      
        ...
        for(int caseCount = 1; caseCount <= n;  caseCount++ ){ 
          ...
          for(int i=1; i<=q; i++){
            ...
            inputType[i] = in.nextInt();
          }
          
          sv.build(1, m, 1);
          
          for(int i=q; i>0; i--)  //this is important!!
            sv.update(inputStart[i], inputEnd[i], inputType[i], 1, m, 1);
