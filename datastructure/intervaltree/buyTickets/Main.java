 ...
 * Solution #3:  example:  " 1 51 and 1 33",  the final position of 1 is decided by the "1 33" instead of "1 51",  so we can calculate the position from the last Posi+Vali to the first Posi+Vali.
...
public class Main
{
  ...
  private void build(int l, int r, int rt) {
    ...
    int mid = (l + r) >> 1;
    build(l, mid, rt << 1);// build(lson);
    build(mid + 1, r, rt << 1 | 1);// build(rson);
  }
