...
public class LargestPalindromic
{
   ...
   */
  public static String largetPalindrome_21(String str){
    ...
    String largetP = "", currMaxP = "";
    for( int pivot = 0; pivot<str.length(); pivot++){
      
      currMaxP = largetPalindrome(str, pivot, pivot);
      if(currMaxP.length() > largetP.length())
        largetP = currMaxP;
      
      currMaxP = largetPalindrome(str, pivot, pivot+1);
      if(currMaxP.length() > largetP.length())
        largetP = currMaxP;
  ...
  private static String largetPalindrome(String str, int pivot1, int pivot2){
    int left=-1, right = str.length();
    while(pivot1 > left && pivot2 < right ){
      if(str.charAt(pivot1) == str.charAt(pivot2)){
        pivot1 --;
        pivot2 ++;
      }else
        break;        
    ...
    return str.substring(pivot1+1, pivot2);
  }
  ...
  public static void main(String[] args) {
    String[] ss = {null, "", "a", "aa", "aaa", "aaaa", "aaaaa", "aaaaaa", "ab", "aba", "abba", "12212321", "waabwswfd", "gabaabaf", "gababaf"};
   
    for(int i=0; i< ss.length; i++){
      System.out.println("\n " +i+ "input: " + ss[i]);	
      System.out.println(" the longest palindormic substring of "+ ss[i] + " is: " +largetPalindrome_Manacher(ss[i]));
