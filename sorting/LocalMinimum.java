...
public class LocalMinimum
{
  ...
  /*
   * find all LM in 1 dimension
   */
  public void findAllLM(int[] arr){
    //check
    if(arr == null || arr.length == 0 )
      return ;
    
    boolean[] isLM = new boolean[arr.length]; // default they are false
    findAllLM_recursive(arr, isLM, 0, arr.length - 1);
    
  }
  
  /**
   *  T(n) = 2T(n/2) + c  so T(n) = O(n), same as travel the arr one by one from left to right. 
   *  
   *  
   * */
  private void findAllLM_recursive(int[] arr, boolean[] isLM, int start, int end){
    if(end - start < 2)
      return;
    ...
    findAllLM_recursive(arr, isLM, start, mid);
    findAllLM_recursive(arr, isLM, mid, end);    
  }
